...
fun _hasSize(plant: AssertionPlant<out Collection<*>>, size: Int): Assertion
    = AssertImpl.collector.collect(plant) {
        property(Collection<*>::size) { toBe(size) }
    }

fun _isEmpty(plant: AssertionPlant<out Collection<*>>): Assertion
    = AssertImpl.builder.createDescriptive(DescriptionBasic.IS, RawString.create(EMPTY)) { plant.subject.isEmpty() }

fun _isNotEmpty(plant: AssertionPlant<out Collection<*>>): Assertion
